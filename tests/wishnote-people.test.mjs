...
import {applyPeople,peoplePrompt} from '../prototype/people.js';
...
test('组件构建保持六条叙事原样，配置与栏目对齐',async()=>{
 ...
 const script=JSON.parse(await read(base+'缄愿笔记_角色脚本.json')),config=script.data.lorestate_text_prototype_v1;
 assert.deepEqual(config.fields,wishnoteFields);assert.equal(config.book,wishnoteBookName);assert.equal(config.uid,6);assert.equal(config.memoryMode,'people');
 assert.deepEqual([...config.html.matchAll(/data-lore-field="([^"]+)"/g)].map(m=>m[1]),wishnoteFields);
 assert.equal(script.export_with.data,true);assert.ok(script.content.includes('@prototype-v0.3.0/'));
 for(const [name,display] of [['显示隐藏',true],['提示词过滤',false]]){
  ...
  const raw='正文保留。<LoreState mode="delta"><Person id="P01" mode="delta" presence="cold"/></LoreState>';
...
test('旧热档字段与冷档原文完整保留，可接着唤醒更新',()=>{
 ...
 assert.ok(!peoplePrompt('记录愿效',wishnoteFields,{state:result.state,errors:[]}).includes('长期负责港口邮路'));
 const wake=applyPeople(result.state,'<LoreState mode="delta"><Person id="P02" mode="delta" presence="active"/></LoreState>',wishnoteFields);
