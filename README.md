...
当前维护线是 **Tavern Helper 文字状态脚本**。当前固定稳定版为不可移动标签 **`0.13.1`**，`main` 是唯一长期开发分支。

## 0.13.1 修复

- **核对规则来源收敛**：字段是否变化只由状态栏条目和自定义更新核对规则决定；内置文字只负责协议、读取权限和格式。
- **精确核对路径**：`LoreStateReview` 会拿到当前 schema 的真实 path 清单，`当前时间` / `当前地点` 等作者栏目不再被示例诱导成 `时间` / `地点`。
- **重试纠错**：失败重试明确要求逐字复制合法 path，避免连续重复同一非法路径。

完整修复与验证边界见 [0.13.1 发布说明](docs/发布材料/0.13.1-更新核对路径修复.md)。
...
固定安装使用 [0.13.1 远程组件](prototype/dist/0.13.1/lorestate-script.json)。只有明确需要跟随开发时才使用 [main 远程组件](prototype/dist/main/lorestate-script.json)。不要同时启用固定版和 main。

也可以在 Tavern Helper 角色脚本中使用固定 import：

```js
import 'https://testingcf.jsdelivr.net/gh/ZZZdragondYNGPHX/LoreState@0.13.1/artifact/bundle.js';
```
...
- **[发布材料](docs/发布材料/README.md)**：当前 0.13.1 对外材料及历史版本发布证据。
