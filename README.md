...
当前维护线是 **Tavern Helper 文字状态脚本**。当前固定稳定版为不可移动标签 **`0.12.3`**，`main` 是唯一长期开发分支。
...
固定安装使用 [0.12.3 远程组件](prototype/dist/0.12.3/lorestate-script.json)。只有明确需要跟随开发时才使用 [main 远程组件](prototype/dist/main/lorestate-script.json)。不要同时启用固定版和 main。
...
- **[发布材料](docs/发布材料/README.md)**：当前 0.12.3 对外材料及历史版本发布证据。
