...
当前维护线是 **Tavern Helper 文字状态脚本的 `main` 开发分支**，当前稳定版为不可移动标签 **`0.12.2`**。历史 `0.10.3` RC 已从误建的版本号 branch 原样固化为同名兼容 tag；它不再接受提交。
...
- **[作者文档目录](docs/README.md)**：当前 0.12.2 入门、模块条目、HTML 与试卡清单。
...
- **[EJS / 动态世界书](docs/EJS动态世界书.md)**：只读桥接、真实字段路径、渐进关系示例与验收边界（0.12.0 起提供，当前稳定入口 0.12.2）。
- **[隐藏助手兼容与更新](docs/隐藏助手兼容与更新.md)**：已有聊天恢复、脚本更新和验收。
- [当前脚本代码](artifact/bundle.js)：`main` 的生成运行代码。
- [常见问题与试卡清单](docs/常见问题与试卡清单.md)。
- [发布材料](docs/发布材料/README.md)：当前 0.12.2 对外帖子、附件和作者改造教程。

## 下载与版本边界

| 入口 | 性质 | 说明 |
| --- | --- | --- |
| [0.12.2 固定远程组件](prototype/dist/0.12.2/lorestate-script.json) | 当前稳定版 | 固定引用 `@0.12.2/artifact/bundle.js`；不随 `main` 变化 |
...
固定安装使用 `0.12.2`；需要跟随开发时才使用 `@main`。固定版本入口不得引用 `@main`。

```js
import 'https://testingcf.jsdelivr.net/gh/ZZZdragondYNGPHX/LoreState@0.12.2/artifact/bundle.js';
```

首次使用 0.12.1 时，先在设置选择世界书与状态栏条目并点击 **确认绑定状态栏条目**，再进入外观制作；最后保存 HTML 并启用当前聊天。

从 **0.11.0 / 0.11.1 / 0.12.0 → 0.12.2** 无需重制合法 v2 模板或迁移 v3 状态；已有模块配置、聊天与快照可以保留。从更早版本升级时需重新制作并应用 Template API v2 HTML；旧模板原文保留，未换模板时显示升级提示，状态引擎继续运行。跨更旧版本升级到模块结构时请先阅读当前作者文档，并把旧 0.9.0 指南只作为历史设计参考。
...
不内置 EJS、不写状态、不自动改世界书；生成前态、旧楼层、切聊天、回档与历史缺口都有独立边界。**0.12.0 首次提供本功能；当前稳定安装使用 0.12.2，接口保持兼容。旧 0.11.1 标签不包含桥接且不会被改写。** 真实酒馆与实际模型仍须单独验收。安装条件、可复制示例与固定上游证据见 [EJS 指南](docs/EJS动态世界书.md)。
...
`npm run build` 生成 `main` 开发入口。准备新的固定版本时，在创建 tag **之前**传入新的版本号。当前 0.12.2 已发布，下一 patch 示例：
