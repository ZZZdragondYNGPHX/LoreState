...
- 活动构建只生成 `artifact/bundle.js`、`prototype/dist/main/lorestate-script.json` 和 `prototype/dist/main/receipt.json`；远程入口指向 `@main`，用户刷新脚本即可取得推送后的新内容。
- `main` 没有需要递增的独立版本号；`package.json` 的旧原生扩展版本不代表 prototype 远程版本。固定标签仅在明确发布新稳定版时递增，旧标签不移动。
- 旧固定版本目录及旧离线产物保留为历史复现，已从当前活动文档和构建流程中移除；本轮不改写旧标签。
- 待验证：本地构建/检查/测试、远程 main 树和 WishNote 分支删除；真实酒馆、CDN 实际加载与人工验收仍待后续。

---

# 历史记录：隐藏楼层兼容修复与 v0.9.2 分发 · 2026-09-10
