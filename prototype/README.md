...
随正文更新失败后，在诊断页点击 **重新计算本层状态**，使用 API 预设中的状态模型重新判断最新回复状态，不切换随正文模式。仅支持此前历史无缺口、标签边界可识别的最新 AI 回复；正文保留，失败不写回，支持取消和撤销最近一次状态更新。基础格式修复已移除，旧备份仍可查看与恢复。详见[额外模型与 API 预设](../docs/额外模型与API预设.md)。
