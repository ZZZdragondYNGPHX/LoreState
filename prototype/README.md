...
- 0.12.3 起：彻底删除旧配置后重建栏目；Chat Completions / Responses / Anthropic 直连；酒馆提示词预设桥接；auto / max / ultra 等思考程度。
- **0.13.0**：更新前 `LoreStateReview` 核对、自定义核对规则、作者默认档案与可移植更新默认值、模块规则副本和角色卡导出准备。
...
状态请求现在先生成简短核对摘要，再提交 v3 状态更新；摘要与实际变化不一致时整轮拒绝。作者可保存自定义核对规则、作者默认初始档案和字段约束。

准备分享角色卡时，使用 **规则配置 → 角色卡导出 → 检查并准备角色卡导出**。该步骤同步当前模块规则副本、校验 HTML/作者默认档案、保存可移植更新默认值并刷新角色局部过滤正则。详见 [更新核对与随卡导出](../docs/更新核对与随卡导出.md)。
