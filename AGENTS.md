...
- 这是 LoreState 的主仓库；当前维护线是 `prototype/` 的 Tavern Helper 远程脚本，`artifact/bundle.js` 是 `main` 分支运行代码；当前固定稳定版是不可移动标签 **`0.12.3`**。
...
- 正式版本只能用不可移动的语义版本 Git tag 表示，例如当前 `0.12.3` 和未来新的 patch tag。**禁止创建裸版本号 branch**，禁止移动或覆盖已发布 tag。
