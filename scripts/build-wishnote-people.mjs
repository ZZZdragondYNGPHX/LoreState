...
import {wishnoteFields,wishnoteBookName} from '../examples/wishnote/people/fields.js';
...
if(!reference.content.includes('@prototype-v0.3.0/'))throw new Error('重新核对人物记忆运行时版本后再构建');
const config={version:3,memoryMode:'people',ready:true,book:wishnoteBookName,uid:6,entryName:book.entries['6'].comment,fields:wishnoteFields,html,presets:[{id:'wishnote-default',name:'缄愿笔记 · 墨金',html}],activePresetId:'wishnote-default'};
const script={...reference,name:'LoreState · 缄愿笔记人物记忆版',id:'512f78d9-3720-4e2c-9762-4f089b11dd20',info:'缄愿笔记技术迁移 1.0。依赖酒馆助手与 LoreState prototype-v0.3.0；导入并绑定配套世界书，首次打开魔法棒设置后保存以安装正则。旧维护 QR、旧 WishNote 扩展和其他 LoreState 脚本须停用。',data:{lorestate_text_prototype_v1:config}};
...
await writeFile(new URL('receipt.json',out),JSON.stringify({version:'1.0',sourceSha256:createHash('sha256').update(source).digest('hex'),preservedEntries:[0,1,2,3,4,5],runtime:'prototype-v0.3.0',wholeCharacterCard:false,hashes},null,2)+'\n');
