...
const pieces=await Promise.all(['modules','state-review','core','entity-delete','template-display','template','presets','builtin-preset','ui-kit','api-profiles','api-transport','preset-bridge','appearance-authoring','appearance-panel','api-panel','extra-update','card-config','control-center-style','control-center','state-frame','snapshots','snapshot-store','ejs-bridge','runtime'].map(name=>read(`prototype/${name}.js`)));
...
const script={type:'script',enabled:true,name:'LoreState · 文字状态原型',id:'0aa89d30-099d-4cf3-8cfb-d367490ec067',content,info:stable?`LoreState ${ref} 固定版本；模块条目、分类栏目与按需规则注入；正文后统一更新。保留 v3 世界实体冷热档、有限关联召回与活动事件；目标酒馆助手 4.9.5 / SillyTavern 1.18.0。首次 full，后续 delta。此入口固定到 ${ref} tag，不随 main 变化。`:'LoreState main 远程开发版；模块条目、分类栏目与按需规则注入；正文后统一更新。保留 v3 世界实体冷热档、有限关联召回与活动事件；本版待真实酒馆验收，目标酒馆助手 4.9.5 / SillyTavern 1.18.0。首次 full，后续 delta。不迁移旧状态；变更栏目可在聊天维护中彻底删除旧配置后重新启用，不迁移旧状态。',button:{enabled:true,buttons:[{name:'LoreState 设置',visible:false}]},data:{},export_with:{data:true,button:true}};
...
const remoteScript={...script,content:`import '${remoteUrl}';`,info:script.info+' 支持更新前核对、自定义随卡核对规则、规则副本与导出准备；作者初值/字段约束及更新默认值随脚本数据导出。保留配置重置、原生多协议 API、酒馆提示词预设桥接与思考程度。 可选 EJS 只读桥接：启用 ST-Prompt-Template 后可在动态世界书读取 LoreState；不内置 EJS、不写回状态。 Template API v2：API 辅助外观制作、字段条件显隐与 class 映射；0.11.0 的 v2 HTML 保持兼容，更早的旧 HTML 需重制，原文与现有状态/schema/快照保留。未完成本版真实宿主与人工验收。'+(stable?'':' 远程入口跟随 main；刷新脚本后获取 main 最新 bundle。')};
await writeFile(new URL(out+'lorestate-script.json',root),JSON.stringify(remoteScript,null,2)+'\n');
await writeFile(new URL(out+'receipt.json',root),JSON.stringify({version:ref,mode:'remote-only',realHostVerified:false,scriptId:script.id,sha256:createHash('sha256').update(content+'\n').digest('hex'),runtime:{sillytavern:'1.18.0',tavernHelper:'4.9.5'},runtimeDependencies:['Tavern Helper'],optionalRuntimeDependencies:[{name:'ST-Prompt-Template',capability:'prompt_template_prepare',sourceVersion:'1.17.9',sourceRevision:'d6f520d149aba146305b0b781ddd691d449c28d2'}],remoteLoaders:[{url:remoteUrl,ref}],remoteEntry:'lorestate-script.json'},null,2)+'\n');
