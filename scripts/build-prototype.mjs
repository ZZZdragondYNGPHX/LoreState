...
const pieces=await Promise.all(['modules','core','entity-delete','template-display','template','presets','builtin-preset','ui-kit','api-profiles','api-transport','preset-bridge','appearance-authoring','appearance-panel','api-panel','extra-update','control-center-style','control-center','state-frame','snapshots','snapshot-store','ejs-bridge','runtime'].map(name=>read(`prototype/${name}.js`)));
...
const remoteScript={...script,content:`import '${remoteUrl}';`,info:script.info+' 支持配置重置、原生多协议 API、酒馆提示词预设桥接与思考程度。 可选 EJS 只读桥接：启用 ST-Prompt-Template 后可在动态世界书读取 LoreState；不内置 EJS、不写回状态。 Template API v2：API 辅助外观制作、字段条件显隐与 class 映射；0.11.0 的 v2 HTML 保持兼容，更早的旧 HTML 需重制，原文与现有状态/schema/快照保留。未完成本版真实宿主与人工验收。'+(stable?'':' 远程入口跟随 main；刷新脚本后获取 main 最新 bundle。')};
