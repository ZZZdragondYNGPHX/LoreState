...
当前稳定脚本为 `prototype-v0.9.2`，包含隐藏楼层兼容修复：隐藏助手不再导致本地状态回放丢失初始化。`main` 分支是持续更新入口，可能随开发变化；日常使用建议固定版本。

**安装**

需要酒馆助手。可以导入下面的脚本 JSON，也可以在酒馆助手中新建角色脚本，填入：
...
[离线备用版 JSON · v0.9.2](https://github.com/ZZZdragondYNGPHX/LoreState/raw/refs/tags/prototype-v0.9.2/prototype/dist/v0.9.2/lorestate-script-offline.json)
...
这是原型试用版。隐藏楼层兼容已通过离线及模拟宿主回归，真实酒馆和实际模型仍需作者验收。欢迎先用测试卡尝试，反馈时附上脚本版本、操作步骤和去除隐私后的错误标签。
