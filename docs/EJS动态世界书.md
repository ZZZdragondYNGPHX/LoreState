# EJS / 动态世界书只读桥接 · 当前 0.12.3

EJS 只读桥接从固定版本 **`0.12.0`** 首次提供；当前固定稳定版 **`0.12.3`** 继续兼容同一接口，并额外修复首次条目绑定/HTML 制作流程。正常安装请使用 `0.12.1`；`0.12.0` 发布说明保留为本功能的首发证据。
...
桥接首发的固定源码、测试数量、上游证据和发布边界见 [0.12.0 发布说明](发布材料/0.12.0-EJS动态世界书.md)；当前固定版的条目绑定修复见 [0.12.3 发布说明](发布材料/0.12.1-条目绑定修复.md)。

## 实机验收建议

至少检查：

1. 当前实际加载的是 `0.12.3` 还是 `main`；
