...
EJS 只读桥接从固定版 **`0.12.0`** 首次提供；当前固定稳定版 **`0.12.3`** 继续兼容同一接口。正常安装请使用 `0.12.3`，不要因为功能首发于 0.12.0/0.12.1 就回退旧标签。
...
首发证据见 [0.12.0 发布说明](发布材料/0.12.0-EJS动态世界书.md)；当前稳定版改动见 [0.12.3 发布说明](发布材料/0.12.3-配置重置与多协议API.md)。
