...
Template API v2 从 `0.11.0` 引入；`0.11.1` 增加条件显隐、class 映射和外观制作台；当前固定版 **`0.12.3`** 继续兼容现有 v2 模板，并修复首次条目绑定与首次 HTML 制作流程。
...
从 `0.11.0` / `0.11.1` / `0.12.0` 升级到 `0.12.2`，合法 v2 模板无需重制，状态/schema/快照也不迁移。
